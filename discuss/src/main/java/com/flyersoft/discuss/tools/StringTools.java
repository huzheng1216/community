...
public class StringTools {
	 ...
	 * @return
	 */
	public static String differTimeGMT(String time,int type){
		...
		if(type == 1){
			...
			try {
				Date date = defaultFormat.parse(time);
				cal1.setTime(date);
			} catch (ParseException e) {
				e.printStackTrace();
			}
		...
		int year = cal1.get(Calendar.YEAR);//获取年份
		int month=cal1.get(Calendar.MONTH);//获取月份
		int day=cal1.get(Calendar.DATE);//获取日
		//int hour=cal1.get(Calendar.HOUR_OF_DAY)-8;//小时
		int hour=cal1.get(Calendar.HOUR_OF_DAY);//小时
		int minute=cal1.get(Calendar.MINUTE);//分
		int second=cal1.get(Calendar.SECOND);//秒

		int c_year = cal2.get(Calendar.YEAR);//获取年份
		int c_month=cal2.get(Calendar.MONTH);//获取月份
		int c_day=cal2.get(Calendar.DATE);//获取日
		int c_hour=cal2.get(Calendar.HOUR_OF_DAY);//小时
		int c_minute=cal2.get(Calendar.MINUTE);//分
		int c_second=cal2.get(Calendar.SECOND);//秒

		if(c_year == year){
			if(c_month == month){
				if(c_day == day){
					long theTime = cal1.getTimeInMillis();
					long differTime = currentTime - theTime;
					long differ = 0;
					if ((differ = differTime / 60000) < 60) {
						str = differ + "分钟前";
	...
	public static String waterwallTime(long time) {
		...
		if (differTime > 0) {
			...
			if ((differ = differTime / 60000) < 60) {
				...
				// } else if ((differ = differTime / 86400000) >= 1
				// && (differ = differTime / 86400000) <= 3) {
				// sb.append(differ).append("天前").toJson();
				// } else if ((differ = differTime / 86400000) > 3) {
				// // return time.substring(5, time.lastIndexOf("-") + 3);
				// return "";
			}
